...
function startSyphon(): void {
  const win = new BrowserWindow({
    ...
    webPreferences: {
      offscreen: { useSharedTexture: true }, // ← MUST be nested like this
...
`output.skipWhenNoClients = true` (default) means no GPU work happens until a client actually connects.
...
| Output is upside-down | Toggle `output.flipY`. |
| Output is 2× the expected size | Retina scaling on Electron < 42. Upgrade and set `offscreen.deviceScaleFactor: 1`, or render at half size. `outWidth`/`outHeight` report the real size. |
