...
**Zero-copy `direct` mode.** `new CompositeSyphonOutput(name, { direct: true })` blits each tile *straight into Syphon's own published surface* (via Syphon's `SyphonSubclassing` API) instead of into an intermediate atlas that Syphon then copies — one blit per frame instead of two. Measured **1.3–1.4× faster** than the default atlas backend (9-out 1.39×, 16-out 1.40×, 25-out 1.29×). The catch: a blit can't mirror, so `direct` forces `flipY = false` — your sources must already be bottom-left oriented (see the flip snippets above). Everything else (partial updates, coalescing, `maxPublishRate`) works unchanged; `DIRECT=1 npm run test:composite` runs the end-to-end check.
